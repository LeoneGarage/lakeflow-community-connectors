...
- **Cross-origin pagination.** The connector refuses to follow an `@odata.nextLink` (or any other URL) whose scheme/host/port differs from `service_url`: the credential-bearing session must never send its `Authorization` header off-origin (a server-supplied off-host link would otherwise exfiltrate the credential). A service that legitimately paginates across hosts is unsupported.
