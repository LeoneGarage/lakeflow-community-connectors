    ...
    body={
        ...
        "options": {
            ...
            "externalOptionsAllowList": (
                ...
                "exclude_ancestor_columns,cursor_lookback_seconds,cursor_probe,contained_fetch"
            ),
        },
    },
)
...
**Capability-verdict caching.** The preflight verdicts above (`expand_ok`, `batch_ok`, `batch_size_ok`, `or_filter_ok`, `cursor_probe_ok`) live in up to three layers, consulted in order: the **resume offset** (streaming cursor reads — durable across restarts via the checkpoint), the **connector instance** (dedupes within one read), and a **process-wide cache keyed by `service_url`** with an on-disk mirror (tempdir JSON, 15-minute TTL, plain booleans only). The last layer exists for reads whose offsets *can't* carry verdicts — contained **snapshot** streams keep their terminal offset a bare `{}` so the stream can quiesce on `end == start`, and the **batch reader** (pipeline snapshot refresh / full refresh) discards offsets entirely — which would otherwise re-run their preflights on every framework-recreated instance, i.e. every microbatch or trigger. Only **definitive** verdicts are recorded at any layer (a transient blip — or a race-contaminated `cursor_probe` sample — records nothing and re-probes next batch); `cursor_probe_ok` joins the shared cache only under the `auto` cascade (the strict `nested-expand` mode neither consults nor records it). The on-disk mirror is rewritten atomically (temp file + rename) so a concurrent forked worker never reads a half-written file, and re-parsed only when its mtime changes (otherwise the hot lookup is a single `stat`). The reset rule is preserved across all read shapes: the **per-table** verdicts (`expand_ok` / `cursor_probe_ok`) are purged from the process/file cache — scoped to just that table — on the next read whenever their governing option is non-`auto`, which covers the bare-offset snapshot and batch-reader paths the offset scrub can't see; the **server-wide** `$batch` verdicts are purged on the offset transition (conservatively, so a sibling table's live `auto` consumer isn't disturbed). Either way, re-selecting `auto` re-runs the preflight rather than resurrecting the old verdict.
...
the nested response into leaf rows tagged with all ancestor FKs. Some
