...
| `retry_extra_errors`          | *(unset)* | Extra HTTP errors to retry, for a source that spuriously fails a well-formed request under load (e.g. Hexagon SCApi). Two forms: a comma-separated list of status codes (`"400,403"` — retry any response with those statuses), or a JSON array of `{"code", "signature"}` objects (retry when the status matches **and**, if `signature` is given, the response body contains it, case-insensitive) — e.g. `[{"code":400,"signature":"context uri"},{"code":403}]`. Matching responses join the same backoff path as the built-in transient statuses, capped by `max_retries`. Codes must be 400–599 and not 404/410 (those drive contained-walk vanished-parent recovery). Off by default: only 408/429/5xx + network errors are retried and 400/403 stay terminal, so genuine client errors surface immediately for well-behaved sources. |
