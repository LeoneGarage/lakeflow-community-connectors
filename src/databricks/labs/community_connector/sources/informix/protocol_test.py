...
class FrameTests(unittest.TestCase):
    def test_cdc_routines_are_cross_database_qualified(self):
        self.assertEqual(cdc_routine("cdc_opensess"), "syscdcv1:informix.cdc_opensess")
    ...
    def test_ordinary_decimal_with_omitted_scale_accepts_null(self):
        column = ResultColumn("prefix", 0, 5, 0, 0x05FF)

        self.assertIsNone(_decode_result_value(b"\0" * 4, column, "utf-8"))

    def test_variable_scale_decimal_preserves_native_exponent(self):
        self.assertEqual(
            decode_value(
                memoryview(b"\xc1\x0c\x22\x00"),
                ColumnDescriptor("amount", "DECIMAL", precision=4, scale=0xFF),
            ),
            (Decimal("12.34"), 4),
        )
    ...
    def test_int8_signed_magnitude_and_datetime_year_to_fraction5(self):
        ...
        raw = bytes([0xC7, 20, 24, 1, 2, 3, 4, 5, 12, 34, 50])
        self.assertEqual(
        ...
        self.assertEqual(
            decode_value(memoryview(bytes([0xC3, 12, 34, 56])), time_only),
            ("12:34:56", 4),
        )
        time_fraction = ColumnDescriptor("t", "DATETIME", length=0x060F)
        self.assertEqual(
            decode_value(memoryview(bytes([0xC2, 33, 11, 31, 77, 30, 0])), time_fraction),
            ("00:33:11.31773", 7),
        )
    ...
    def test_datetime_partial_qualifiers(self):
        cases = (
            ...
            (0x0408, bytes([0xC3, 12, 3, 4]), "DAY=12:HOUR=03:MINUTE=04"),
            (0x0002, bytes([0xC3, 20, 24, 1]), "YEAR=2024:MONTH=01"),
            (0x080F, bytes([0xC2, 34, 56, 12, 34, 50]), "MINUTE=34:SECOND=56:FRACTION(5)=12345"),
        )
    ...
    def test_datetime_partial_qualifiers_reject_out_of_range_fields(self):
        cases = (
            (0x060A, bytes([0xC3, 24, 0, 0])),
            (0x080A, bytes([0xC2, 60, 0])),
            (0x0A0A, bytes([0xC1, 60])),
            (0x0204, bytes([0xC2, 13, 1])),
            (0x0404, bytes([0xC1, 32])),
        )
        for qualifier, raw in cases:
            with self.subTest(qualifier=qualifier):
                with self.assertRaisesRegex(CdcProtocolError, "invalid DATETIME field"):
                    decode_value(
                        memoryview(raw),
                        ColumnDescriptor("t", "DATETIME", length=qualifier),
                    )

    def test_partial_month_day_uses_calendar_bounds(self):
        descriptor = ColumnDescriptor("t", "DATETIME", length=0x0204)
        with self.assertRaisesRegex(CdcProtocolError, "calendar fields"):
            decode_value(memoryview(bytes([0xC2, 2, 30])), descriptor)
        self.assertEqual(
            decode_value(memoryview(bytes([0xC2, 2, 29])), descriptor),
            ("MONTH=02:DAY=29", 3),
        )
    ...
    def test_ordinary_boolean_null_and_marker_validation(self):
        ...
        self.assertTrue(
            _decode_result_value(b"\x00" + struct.pack(">i", 1) + b"\x01", enveloped, "utf-8")
        )
        self.assertIsNone(_decode_result_value(b"\x01" + struct.pack(">i", 0), enveloped, "utf-8"))
    ...
    def test_ordinary_lvarchar_value_null_and_validation(self):
        ...
        self.assertIsNone(_decode_result_value(b"\x01" + struct.pack(">i", 0), column, "utf-8"))
        with self.assertRaisesRegex(SqliProtocolError, "length"):
            _decode_result_value(b"\x00" + struct.pack(">i", 4) + b"short", column, "utf-8")
...
class SqliPacketTests(unittest.TestCase):
    def test_unbounded_execute_skips_decoded_byte_accounting(self):
        ...
        with (
            patch.object(client, "_require_open", return_value=(io.BytesIO(), output)),
            patch.object(client, "_read_query_group", return_value=(None, [], False)),
            patch(
    ...
    def test_execute_routes_non_row_statements_to_command_path(self):
        ...
        with patch.object(client, "execute_command") as execute_command:
            self.assertEqual(client.execute("UPDATE app.orders SET state = ?", ("done",)), [])

        execute_command.assert_called_once_with("UPDATE app.orders SET state = 'done'")
    ...
    def test_connector_sql_renders_date_and_datetime_pagination_values(self):
        self.assertEqual(
            _connector_sql(
                "SELECT * FROM t WHERE d > ? AND ts > ?",
                (date(2026, 7, 24), datetime(2026, 7, 24, 1, 2, 3, 123450)),
            ),
            "SELECT * FROM t WHERE d > MDY(7,24,2026) AND ts > "
            "DATETIME(2026-07-24 01:02:03.12345) YEAR TO FRACTION(5)",
        )
        with self.assertRaisesRegex(ValueError, "timezone-free"):
            _connector_sql("SELECT * FROM t WHERE ts > ?", (datetime.now(timezone.utc),))

        self.assertEqual(
            _connector_sql(
                "SELECT * FROM t WHERE t > ?",
                (InformixDatetimeLiteral("13:03:36.93100", 0x060F),),
            ),
            "SELECT * FROM t WHERE t > " "DATETIME(13:03:36.93100) HOUR TO FRACTION(5)",
        )

    def test_connector_sql_renders_bounded_finite_decimal_pagination_values(self):
        self.assertEqual(
            _connector_sql(
                "SELECT * FROM t WHERE amount > ?",
                (Decimal("123.4500"),),
            ),
            "SELECT * FROM t WHERE amount > 123.4500",
        )
        with self.assertRaisesRegex(ValueError, "must be finite"):
            _connector_sql("SELECT * FROM t WHERE amount > ?", (Decimal("NaN"),))
    ...
    def test_execute_enforces_incremental_decoded_result_byte_bound(self):
        client = InformixSqliClient("host", 9088, "db", "user", "password")
        description = ResultDescription(2, 7, 4, (ResultColumn("value", 0, 2, 0, 4),))
        responses = (
            (description, [], False),
        ...
        with (
            patch.object(client, "_require_open", return_value=(io.BytesIO(), output)),
            patch.object(client, "_read_query_group", side_effect=responses),
            patch.object(client, "_poison"),
            self.assertRaisesRegex(SqliProtocolError, "max_result_bytes=1"),
        ):
    ...
    def test_close_sends_graceful_sqli_exit_for_open_database(self):
        client = InformixSqliClient("host", 9088, "db", "user", "password")
        output = Mock()
        client._input = Mock()
        ...
        output.write.assert_called_once_with(struct.pack(">hh", SQ_EXIT, SQ_EOT))
        output.flush.assert_called_once_with()
        self.assertEqual(client.state, ConnectionState.CLOSED)

    def test_close_preserves_poisoned_state(self):
        client = InformixSqliClient("host", 9088, "db", "user", "password")
        client.state = ConnectionState.POISONED

        client.close()

        self.assertEqual(client.state, ConnectionState.POISONED)
    ...
    def test_pam_multiround_information_and_accept(self):
        wire = (
            struct.pack(">hhh", 129, 4, 4)
            + b"info"
            + struct.pack(">h", 12)
            + struct.pack(">hhh", 129, 1, 9)
            + b"Password:\0"
            + struct.pack(">h", 12)
    ...
    def test_pam_encoded_response_bound_and_round_limit(self):
        ...
        client = InformixSqliClient(
            ...
            client_locale="en_US.utf8",
            authentication_mode="pam",
            authentication_provider=PasswordAuthenticationProvider("x" * 513),
            pam_max_rounds=1,
        )
        client._input, client._output = io.BytesIO(wire), io.BytesIO()
        with self.assertRaises(SqliUnsupportedAuthentication):
    ...
    def test_redirect_shared_address_space_requires_exact_ip_allowlist(self):
        ...
        with self.assertRaises(SqliUnsupportedAuthentication):
            client._validate_redirect_destination("target", 9088, True, ("100.64.0.1",))
        client.redirect_allowlist = frozenset({("target", 9088), ("100.64.0.1", 9088)})
        client._validate_redirect_destination("target", 9088, True, ("100.64.0.1",))
    ...
    def test_plaintext_connection_does_not_create_or_wrap_tls_context(self):
        ...
        with (
            ...
            patch("ssl.create_default_context") as create_context,
            self.assertRaisesRegex(RuntimeError, "stop after socket selection"),
        ):
            client._connect_once("host", 9088, "server", float("inf"))
    ...
    def test_accept_and_redirect_states(self):
        ...
        with self.assertRaisesRegex(SqliSessionRejected, "server log"):
            decode_asc_response(encode_session_packet(3, b""))
        with self.assertRaisesRegex(SqliSessionRejected, "server log"):
            decode_asc_accept(encode_session_packet(3, b""))
        with self.assertRaises(SqliUnsupportedAuthentication):
    ...
    def test_mixed_lvarchar_and_boolean_tuple_decodes_variable_envelopes(self):
        payload = (
            b"\x00" + struct.pack(">i", 5) + b"hello" + b"\x00" + struct.pack(">i", 1) + b"\x01"
        )
        ...
        self.assertEqual(client._read_tuple(description), {"text": "hello", "enabled": True})
    ...
    def test_padded_varchar_does_not_disable_lvarchar_envelope_walking(self):
        ...
        if len(payload) & 1:
            transcript += b"\x00"
        client = InformixSqliClient("host", 9088, "db", "user", "password", pad_varchar=True)
        client._input = io.BytesIO(transcript)
        description = ResultDescription(
            2,
            7,
    ...
    def test_query_exit_terminates_response_group(self):
        client = InformixSqliClient("host", 9088, "db", "user", "password")
        client._input = io.BytesIO(struct.pack(">h", 56))

        description, rows, exhausted = client._read_query_group(None)

        self.assertIsNone(description)
        self.assertEqual(rows, [])
        self.assertFalse(exhausted)

    def test_status_exit_terminates_completed_response_group(self):
        done = struct.pack(">hhiii", 15, 0, 0, 0, 0)
        client = InformixSqliClient("host", 9088, "db", "user", "password")
        client._input = io.BytesIO(done + struct.pack(">h", 56))

        client._read_status_group(require_done=True)

        self.assertEqual(client._input.tell(), len(done) + 2)
    ...
    def test_status_group_consumes_insertdone_serial_metadata(self):
        for bigint_supported, body in (
            ...
            with self.subTest(bigint_supported=bigint_supported):
                ...
                transcript = struct.pack(">hh", 8, 94) + body + done + struct.pack(">h", 12)
                client = InformixSqliClient("host", 9088, "db", "user", "password")
                client.bigint_supported = bigint_supported
                client._input = io.BytesIO(transcript)
                with patch.object(
    ...
    def test_full_synthetic_connect_state_machine(self):
        ...
        for tls in (True, False):
            with self.subTest(tls=tls):
                ...
                self.assertEqual(context.calls, 1 if tls else 0)


class DatetimeChunkCastTests(unittest.TestCase):
    def test_contiguous_qualifiers_produce_fixed_width_casts(self):
        # (packed qualifier, expected CHAR width) for the column's own range.
        # Widths are the canonical zero-padded text length: YEAR..SECOND ->
        # 4/7/10/13/16/19, HOUR-anchored TIME -> 2/5/8, plus 1+digits per
        # FRACTION. Casting in the column's own qualifier keeps the string
        # fixed-width, so byte order is a stable total order for keyset paging.
        cases = [
            (0x0000, 4),  # YEAR
            (0x0004, 10),  # YEAR TO DAY
            (0x000A, 19),  # YEAR TO SECOND
            (0x000F, 25),  # YEAR TO FRACTION(5)
            (0x0608, 5),  # HOUR TO MINUTE
            (0x060A, 8),  # HOUR TO SECOND
            (0x060F, 14),  # HOUR TO FRACTION(5)  (tw221's lu_time)
            (0x0204, 5),  # MONTH TO DAY
        ]
        for qualifier, width in cases:
            with self.subTest(qualifier=qualifier):
                expr = datetime_order_preserving_cast("ts", qualifier)
                self.assertIsNotNone(expr)
                # No EXTEND: cast the column in its own declared range.
                self.assertNotIn("EXTEND", expr)
                self.assertEqual(expr, f"CAST(ts AS CHAR({width}))")

    def test_malformed_qualifiers_have_no_cast(self):
        # Non-field start nibble, or end before start: reject so the caller
        # falls back instead of chunking on an ill-defined order.
        for qualifier in (0x0105, 0x0400, 0x0700):
            with self.subTest(qualifier=qualifier):
                self.assertIsNone(datetime_order_preserving_cast("ts", qualifier))


class StaleCaptureLabelFrameTests(unittest.TestCase):
    """Real frames captured from Informix 14.10 for tw316_ovhc via syscdcv1.

    Both frames are INSERT (kind 40) and both carry capture label 1, whose
    registered layout is ``source_rowid INTEGER, payload VARCHAR(160)``. The
    difference is the logical log they came from: the source replayed records
    from a reused log file under this session's label, so the label alone is not
    proof that a payload matches the layout.
    """

    # logical log 12: a genuine tw316_ovhc row.
    GOOD = base64.b64decode(
        "AAAAKAAAACEAAABCAAAAKAAAAAwEpMG4AAAAtgAAAAEAAAAAAAAAHQAAAAEcU3ludGhldGlj"
        "IE9WSEMgaGVhbHRoIGZ1bmQgMQ=="
    )
    # logical log 7: a syscolumns row (tabname/tabid/colno/coltype/collength),
    # nothing to do with tw316_ovhc, but tagged with the same capture label.
    ...
    def test_a_genuine_captured_row_still_decodes(self):
        record = decode_frame(self.GOOD, self.labels())

        self.assertEqual(record["op"], "INSERT")
        self.assertEqual(record["label"], 1)
        self.assertEqual(
            record["row"], {"source_rowid": 1, "payload": "Synthetic OVHC health fund 1"}
        )

    def test_a_stale_labelled_frame_reports_the_column_and_offset(self):
        """The bare UnicodeDecodeError this used to raise named neither.

        decode_row caught only CdcProtocolError, so the codec error from the text
        branch escaped unwrapped and production saw
        "'utf-8' codec can't decode byte 0x80 in position 12" with no indication
        of which column or payload offset failed.
        """

        with self.assertRaises(CdcProtocolError) as caught:
            decode_frame(self.STALE, self.labels())

        message = str(caught.exception)
        self.assertIn("payload", message)
        self.assertIn("offset", message)
        # The underlying codec failure is preserved as the cause.
        self.assertIsInstance(caught.exception.__cause__, UnicodeDecodeError)

    def test_the_stale_frame_carries_the_same_label_as_the_genuine_one(self):
        """Pins the premise: the label cannot distinguish these two frames."""

        good_lsn, _, good_label = struct.unpack_from(">Qii", self.GOOD, 16)
        stale_lsn, _, stale_label = struct.unpack_from(">Qii", self.STALE, 16)
