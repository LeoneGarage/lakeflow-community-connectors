...
class StagedPagePrefetcherTests(unittest.TestCase):
    """The single-slot staged-page prefetcher: holds one page, supersedes on a new
    key, blocks take() until ready, and degrades a prefetch error to a cache miss."""

    def test_take_returns_the_prefetched_rows(self):
        prefetcher = informix_module._StagedPagePrefetcher()
        rows = [{"id": 1}]
        prefetcher.submit(("lsn", 1), lambda: rows)
        self.assertEqual(prefetcher.take(("lsn", 1)), rows)

    def test_take_of_a_different_key_returns_none(self):
        prefetcher = informix_module._StagedPagePrefetcher()
        prefetcher.submit(("lsn", 1), lambda: [{"id": 1}])
        self.assertIsNone(prefetcher.take(("lsn", 2)))

    def test_superseding_submit_discards_the_previous_page(self):
        prefetcher = informix_module._StagedPagePrefetcher()
        prefetcher.submit(("lsn", 1), lambda: [{"id": 1}])
        prefetcher.submit(("lsn", 2), lambda: [{"id": 2}])
        self.assertIsNone(prefetcher.take(("lsn", 1)), "superseded key must be gone")
        self.assertEqual(prefetcher.take(("lsn", 2)), [{"id": 2}])

    def test_prefetch_error_falls_back_to_a_cache_miss(self):
        prefetcher = informix_module._StagedPagePrefetcher()

        def boom():
            raise informix_module.InformixError("cannot read page")

        prefetcher.submit(("lsn", 1), boom)
        # Best-effort: an error surfaces as None (read synchronously), never a raise.
        self.assertIsNone(prefetcher.take(("lsn", 1)))

    def test_take_blocks_until_the_worker_finishes(self):
        prefetcher = informix_module._StagedPagePrefetcher()
        ...
        prefetcher.submit(("lsn", 1), slow)
        self.assertTrue(started.wait(5))
        release.set()
        self.assertEqual(prefetcher.take(("lsn", 1)), [{"id": 9}])


class StagedPagePrefetchTests(unittest.TestCase):
    """Volume page prefetch overlaps the next staged page's read+decode with the
    current page's write -- off by default, and identical to the synchronous serve."""
    ...
    def connector(self, bridge, *, scope="prefetch-pipeline", **options):
        connector = InformixLakeflowConnect(
            {
                "database": "demo",
                "snapshot.staging.location": self._shared_state.name,
        ...
        connector.set_registration_scope(hashlib.sha256(scope.encode()).hexdigest()[:32])
        connector._bridge_instance = bridge
        return connector

    def _instrument_page_loads(self, connector):
        """Record (page_index, thread_name) for every actual page read+decode."""

        loads = []
        real = connector._load_staged_page_rows

        def spy(table, pipeline_scope, schema_id, snapshot_lsn, page_index):
            loads.append((page_index, threading.current_thread().name))
            return real(table, pipeline_scope, schema_id, snapshot_lsn, page_index)

        connector._load_staged_page_rows = spy
        return loads

    def _drain(self, connector):
        offset: dict = {}
        collected = []
        for _ in range(50):
            rows, offset = connector.read_table("app.orders", offset, {})
            collected.extend(rows)
            if offset.get("phase") != "snapshot":
                break
        else:
            self.fail("staged snapshot did not converge")
        return collected, offset

    def test_prefetch_serves_later_pages_from_the_worker_thread(self):
        bridge = FakeBridge()
        bridge.rows = [{"id": i, "value": str(i)} for i in range(1, 5)]  # 4 pages @ size 1
        connector = self.connector(
            bridge, **{"snapshot.page.size": "1", "snapshot.prefetch.enabled": "true"}
        )
        loads = self._instrument_page_loads(connector)

        rows, end = self._drain(connector)

        self.assertEqual([r["id"] for r in rows], [1, 2, 3, 4])
        self.assertEqual(end["phase"], "stream")
        by_page = dict(loads)
        # Page 0 is read synchronously; every later page is prepared on the worker,
        # and take() blocks for it, so this is deterministic.
        self.assertEqual(by_page[0], threading.current_thread().name)
        for page_index in (1, 2, 3):
            self.assertIn("informix-page-prefetch", by_page[page_index])

    def test_prefetch_on_by_default_serves_later_pages_from_the_worker(self):
        # No option set -> the default (true) engages prefetch.
        bridge = FakeBridge()
        bridge.rows = [{"id": i, "value": str(i)} for i in range(1, 4)]
        connector = self.connector(bridge, **{"snapshot.page.size": "1"})
        self.assertTrue(connector._snapshot_prefetch_enabled(), "prefetch defaults on")
        loads = self._instrument_page_loads(connector)

        rows, _ = self._drain(connector)
    ...
    def test_prefetch_can_be_disabled_and_reads_synchronously(self):
        bridge = FakeBridge()
        bridge.rows = [{"id": i, "value": str(i)} for i in range(1, 4)]
        connector = self.connector(
            bridge, **{"snapshot.page.size": "1", "snapshot.prefetch.enabled": "false"}
        )
        loads = self._instrument_page_loads(connector)
        ...
        self.assertEqual([r["id"] for r in rows], [1, 2, 3])
        self.assertIsNone(connector._staged_page_prefetcher, "no prefetcher when disabled")
        main = threading.current_thread().name
        self.assertTrue(
            all(name == main for _, name in loads), "every page read on the main thread"
        )

    def test_prefetch_matches_synchronous_rows_and_final_phase(self):
        def drain_ids(*, prefetch, scope):
            bridge = FakeBridge()
            bridge.rows = [{"id": i, "value": str(i)} for i in range(1, 6)]
            options = {
                "snapshot.page.size": "1",
                "snapshot.prefetch.enabled": "true" if prefetch else "false",
            }
            connector = self.connector(bridge, scope=scope, **options)
            rows, end = self._drain(connector)
            return [r["id"] for r in rows], end["phase"]

        self.assertEqual(
            drain_ids(prefetch=False, scope="sync-parity"),
            drain_ids(prefetch=True, scope="prefetch-parity"),
        )

    def test_final_page_does_not_prefetch_across_the_stream_boundary(self):
        bridge = FakeBridge()
        bridge.rows = [{"id": 1, "value": "a"}, {"id": 2, "value": "b"}]  # 2 pages
        connector = self.connector(
            bridge, **{"snapshot.page.size": "1", "snapshot.prefetch.enabled": "true"}
        )
        loads = self._instrument_page_loads(connector)
        ...
        self.assertEqual(end["phase"], "stream")
        # Only pages 0 and 1 are ever read: the final page serves without submitting
        # a prefetch, so no page-2 read is attempted across the boundary.
        self.assertEqual(sorted(page for page, _ in loads), [0, 1])
...
class SnapshotFairnessOptionTests(unittest.TestCase):
    ...
    def test_reader_threads_override(self):
        # An explicit value wins regardless of pool size.
        self.assertEqual(
            self._connector(
                **{"snapshot.reader.threads": "4", "max.concurrent.connections": "6"}
            )._snapshot_reader_thread_count(),
            4,
        )
...
class DaemonReservationFloorTests(unittest.TestCase):
    ...
    def test_snapshot_daemon_floors_one_band_below_cdc_daemon(self):
        # Pool 6, daemon reservation 2, snapshot.reader.threads pinned to 1 -> the snapshot
        # drain floors at 2 - 1 = 1, one slot below the CDC daemon (which floors at 2). That
        # leaves slot [1, 2) reachable by the drain but NOT the CDC daemon, so a saturated
        # CDC daemon cannot starve the drain. Slot 0 stays for consumer bootstrap. (Pinned
        # to 1 because the default now scales to a third of the pool.)
        bridge = self._bridge(**{"snapshot.reader.threads": "1"})
