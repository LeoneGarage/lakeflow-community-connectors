...
| `snapshot.reader.threads` | No | `1` | **Model C only** (`snapshot.shared.session=true`): number of daemon workers that drain `initial` snapshots concurrently, which is also the ceiling on connection slots snapshots hold at once; minimum `1`. Keep it below `max.concurrent.connections` so streaming/CDC readers always retain slots (a good starting point is `max.concurrent.connections − cdc.shared.reader.threads − a small margin`). |
