...
| `snapshot.staging.retention.days` | No | `4` | Age threshold for the **background retention sweep** that reclaims abandoned snapshot staging; minimum `1`. Normal cleanup removes consumed pages incrementally and the whole stage at the snapshot→CDC transition — this sweep is the safety net for a `runs/<snapshot_lsn>` subtree left behind by a crash, an abandoned resnapshot, or a superseded schema/scope. It runs **asynchronously** (off the read path, on the same background pool as page deletion), at most once per table per reader, and deletes only runs whose newest page file is older than this window, so an active run (recent writes) is never touched. Best-effort: a leftover only wastes Volume space, never affects correctness. |
...
| `connection.wait.timeout.seconds` | No | `600` | Maximum time a microbatch blocks while acquiring an Informix connection-capacity slot; minimum `1`. A **triggered** read that exhausts it fails with `ConnectionCapacityUnavailable` rather than returning an empty successful batch: triggered flows drain and terminate, so they release their slots, pressure is transient, and an empty batch is how `AvailableNow` recognises a drained source. A **continuous** read does not wait this long at all — see `capacity.retry.max.delay.seconds`. It also governs the **shared snapshot drain** (the daemon-pool drain, `snapshot.shared.session=true`) as a *per-progress liveness window*, not a hard cap: the drain worker resets it every time it stages a page **and** — while it is still waiting for its connection slot — every time the pool claims a slot (turnover), so a drain queued behind a busy backfill waits it out rather than failing. The worker gives up only when the pool is genuinely **wedged** (no slot claimed for a whole window), raising `ConnectionCapacityUnavailable` with a message that says the pool made no progress (distinct from a drain that connected but then stalled its scan). The consumer's watchdog fails only if the worker goes dark without recording an error. So raising this makes a drain both more patient waiting out a busy pool and more tolerant of long stalls. See `snapshot.drain.slot.liveness.enabled` to disable the turnover reset. |
| `connection.startup.jitter.seconds` | No | `0` | One-shot jittered delay (fractional seconds) a reader sleeps before its **first** connection, so a pipeline that starts many flows at once does not open all their Informix CDC sessions in the same instant. The syscdcv1 CDC subsystem can stall under many concurrent session open/teardowns at startup, surfacing as a socket read timeout inside `validate_initial_lsn`; spreading the first connects over a window avoids the pile-up. Each reader sleeps a random duration in `[0, this)` exactly once, **before** it acquires a connection slot (so it holds nothing while waiting). Default `0` disables it; steady-state polls and reconnects are never delayed (the delay is one-shot per reader). Set it to a few seconds on a large multi-table pipeline that intermittently fails at startup with `validate_initial_lsn` timeouts. Must be a number `>= 0`. |
| `connection.fair.queue.enabled` | No | `true` | Whether slot acquisition uses the **fair waiter queue**. A reader blocked on a slot enqueues a ticket in Lakebase, so acquisition is **first-come per slot** instead of a raw race — no reader is lapped indefinitely while others churn the pool. Ordering is *per slot*, not global: an older reader reserved to high slots (e.g. the delete channel) never blocks a younger reader from a low slot it could not use anyway, so there is no cross-band head-of-line blocking. Tickets carry the same reservation `floor`/`ceiling` the acquire uses, heartbeat each sweep, and are dropped the instant a reader wins or gives up; a crashed reader's ticket ages out (≈15s) and is reaped by the next heartbeat, so it never wedges the queue. This bounds worst-case waiting under contention but does **not** create capacity — if demand exceeds supply, fair waiters still time out with `ConnectionCapacityUnavailable`, just in order. Set `false` to revert to the unordered race (a kill-switch, no redeploy needed, if the queue ever misbehaves). Accepts `true`/`false` (`1`/`yes`, `0`/`no`). |
| `connection.fair.queue.policy` | No | `round_robin` | How the fair waiter queue orders waiters (only when `connection.fair.queue.enabled=true`). `round_robin` (default) rotates service across **requester classes** — consumer reads, the CDC shard daemon, the snapshot drain — so each class gets a turn no matter how many tickets another class holds. This is the volume-immune fix for the starvation case where a burst of consumer bootstrap/fallback tickets crowds out the few CDC-daemon tickets by sheer count: a per-namespace rotation cursor moves past a class once it is served, so plain per-ticket FIFO cannot let one class monopolise service. `fifo` serves strictly by arrival — fair per ticket, but not per class. Accepts `round_robin`/`fifo`. |
| `snapshot.drain.slot.liveness.enabled` | No | `true` | Whether the **shared snapshot drain worker** treats its connection-slot wait as a liveness watchdog. Default `true`: a drain worker blocked acquiring its slot resets its `connection.wait.timeout.seconds` window every time the pool claims a slot (turnover — other work is completing and recycling capacity), so it waits out a busy backfill and only fails when the pool is genuinely wedged (zero turnover for a whole window). This matters most on a small pool during a large multi-table backfill, where the single keyless drain worker (`snapshot.reader.threads=1`) would otherwise time out against a pool that is full but healthy. Set `false` to revert to a plain fixed deadline — the worker then fails after one `connection.wait.timeout.seconds` window regardless of whether the pool is busy or wedged (a kill-switch, no redeploy needed). Only affects the snapshot-drain daemon; the CDC shard daemon has its own `cdc.shared.slot.liveness.enabled` and consumer reads always keep the plain deadline. Accepts `true`/`false` (`1`/`yes`, `0`/`no`). |
| `cdc.shared.slot.liveness.enabled` | No | `true` | Whether the **CDC shard daemon** treats its connection-slot wait as the same turnover-based liveness watchdog the snapshot drain uses. Default `true`: while actively catching up, a shard daemon blocked acquiring its slot keeps its queue ticket and resets its `connection.wait.timeout.seconds` window on each pool slot claim (turnover), so the round-robin rotation has time to reach it — a round is paid in slot turnover, not wall-clock — instead of failing fast on a short budget and re-queuing at the back (the pattern that let a consumer fallback herd wedge the shared session on a saturated small pool). It gives up only when the pool is genuinely wedged, then degrades its consumers to direct reads. Set `false` to revert to the plain `connection.wait.timeout.seconds` deadline (a kill-switch, no redeploy needed). Accepts `true`/`false` (`1`/`yes`, `0`/`no`). |
| `string.null.byte` | No | `null` | How a decoded string value that contains an embedded **NUL** (`\x00`) is sanitized. Many downstream consumers cannot store a NUL, and it is **genuine content** here — the connector already trims real (space) CHAR padding and VARCHAR is length-delimited, so a NUL that reaches the output was embedded in the value, not padding. `null` (default): a string containing any NUL becomes **NULL** — the honest, downstream-detectable signal that the value was unrepresentable, rather than silently altering it. `empty`: **strip** the NUL characters, preserving the rest of the string. `keep`: pass the NUL through unchanged (previous behavior). Applies to both the CDC and snapshot/query decode paths. ⚠️ **Key-column caveat:** a promoted unique-index / `primary.keys` key column is required non-null; if such a text key value contains a NUL, `null` mode would null it and break keyset pagination / the merge on that row — use `empty` if key columns may carry NULs. Accepts `null`/`empty`/`keep` (`empty_string` is an alias for `empty`). |
| `primary.key.from.unique.index` | No | `true` | Whether a table with **no primary-key constraint** is promoted to **keyed** on a `UNIQUE` index. Default `true`: the connector discovers the table's unique indexes and, if any qualifies, uses its columns as the primary key — so the table takes the resumable keyed path (keyset-paginated snapshots, CDC dedup, `cdc_with_deletes`) instead of the one-shot key-less drain. Selection is deterministic: only indexes whose columns are **all `NOT NULL`** qualify (a primary key must be non-null for keyset pagination and dedup to be sound); among those, the index with the **fewest columns** wins, and ties are broken by **index name in ascending order**. An explicit `primary.keys` option still overrides the discovered key. The discovered key is catalog-derived, so a schema refresh re-derives it. Set `false` to keep the old behavior (only a real primary key makes a table keyed; otherwise it is key-less). Note this changes a table's keyed/key-less classification, which changes its schema fingerprint — enabling it on an already-running key-less table requires a full refresh. Accepts `true`/`false` (`1`/`yes`, `0`/`no`). |
...
| `primary.keys` | No | *(catalog key)* | Per-table override of the table's primary key — a comma-separated list (or JSON array) of column names. When set, the connector treats the table as **keyed** on those columns even if it is physically keyless: it reports `ingestion_type=cdc_with_deletes`, reads incrementally with keyset-paginated snapshots and delete identification, and reports the keys so the pipeline uses them as the destination merge key too (no separate `primary_keys` needed). The columns must exist in the table. **You must guarantee they are unique per row** — a non-unique override corrupts upserts, delete identification, and snapshot pagination. Leave unset to use the catalog's primary key, then a `UNIQUE` index if the connection has `primary.key.from.unique.index` enabled (the default), and otherwise none. |
| `allow.nullable.index` | No | `false` | Per-table opt-in that lets a `UNIQUE` index containing **nullable** column(s) be promoted to the key when the table has **no primary key and no all-`NOT NULL` unique index**. Normally such an index is rejected (a key must be non-null for keyset pagination and CDC dedup); setting this `true` asserts the columns are **unique and non-null for every row** — a real NULL there would mis-order or drop rows. Only takes effect when `primary.key.from.unique.index` is enabled (the default) and no non-null key exists; an all-`NOT NULL` unique index and an explicit `primary.keys` both still win. Promoting a table changes its keyed/key-less classification and schema fingerprint, so enable it before first ingest or run a full refresh. Accepts `true`/`false`. |
...
| `cdc.shared.session` | No | `true` | Sharded daemon-reader CDC (default on; set `false` to opt out). When on, tables are partitioned into K shards and a bounded pool of K driver-resident daemon threads each hold one shared CDC session over their shard's tables, fanning changes out in memory — so concurrent CDC sessions drop from ~N to K and reads run in parallel across the K threads. In steady state a streaming poll reads schema, log bounds, and change records from the shard and touches Informix for nothing. Accepts `true`/`false` (`1`/`yes`, `0`/`no`). Each shard's log-retention floor tracks that shard's slowest table. A table reads from its shard only once a daemon read activated at or below the table's checkpoint has covered it; until then its polls fall back to direct reads, and the daemon never advances the shard's shared restart past an uncovered table's floor — so a table that subscribes (or re-subscribes after the idle TTL) while a read is in flight cannot have the log between its checkpoint and that read's end skipped. |
| `cdc.shared.reader.threads` | No | `max(1, max.concurrent.connections // 4)` | Number of shards / daemon reader threads (K) **per channel** when `cdc.shared.session=true`; minimum `1`. The upsert and delete channels shard independently, so K threads mean up to **2K** daemon connection slots. The default (a quarter of the pool → ~half the pool in total daemon slots) leaves room for consumer bootstrap reads and the snapshot drain; the previous default of `max.concurrent.connections` targeted 2N slots against an N-slot pool and could pin the entire daemon reservation band. K=1 collapses a channel onto one session; larger K spreads tables across more parallel readers at the cost of connection slots. |
| `cdc.shared.buffer.max.records` | No | `200000` | Connection-level option that caps each table's buffered-but-unconsumed transactions in a shard (`cdc.shared.session=true`); minimum `1`. Set on the connection (not per table), it applies the same per-table buffer limit to every table. When a table reaches it, that shard's daemon defers new reads (throttle to the slowest reader) until the backlog drains, bounding driver memory. |
| `snapshot.shared.session` | No | `true` | Selects one of two drain strategies for an `initial`-mode snapshot, both of which stop the full-table drain from starving streaming readers while it holds a connection slot for the whole scan. **Off-thread drain** (default `true`): the drain moves off the microbatch thread onto a bounded pool of `snapshot.reader.threads` daemon workers; the consumer waits — holding no slot — for the daemon to stage the table and publish its manifest, then serves the staged pages exactly as a resumed snapshot does. At most `snapshot.reader.threads` drains hold a slot at once. **Inline drain** (`false`): the drain runs on the microbatch thread but acquires its slot above `snapshot.connection.reservation`, keeping those low slots reachable by streaming/CDC readers. Accepts `true`/`false` (`1`/`yes`, `0`/`no`). |
| `snapshot.reader.threads` | No | `max(1, daemon.connection.reservation − 1)` | **Off-thread drain only** (`snapshot.shared.session=true`): number of daemon workers that drain `initial` snapshots concurrently, which is also the ceiling on connection slots snapshots hold at once; minimum `1`. Keyless (append-only, no primary key) tables drain **one-at-a-time** through this pool, so a single worker serializes every keyless table's initial scan and the later tables sit queued behind it. The default is **one below the CDC-daemon reservation** (`daemon.connection.reservation`, itself `≈ max.concurrent.connections / 3`), so the snapshot-drain floor (`daemon.connection.reservation − snapshot.reader.threads`) never collapses to `0`: a low slot always stays reachable by consumer bootstrap reads even while the drains hold their slots for whole scans — a drain that grabbed the lowest slots and held them would otherwise starve consumer/CDC reads on a small pool. It parallelizes drains only once the pool is large enough to spare a drain band and still keep a consumer floor (e.g. a 6-slot pool → reservation 2 → 1 worker; a 16-slot pool → reservation 5 → 4 workers). A large keyless table is not capped by a total-duration timeout: the consumer waits as long as the drain keeps staging pages (see `connection.wait.timeout.seconds`), so the levers for a big table are a larger pool (which raises this default) or declaring `primary.keys` (moves it onto the resumable keyed path). |
| `snapshot.connection.reservation` | No | `0` | **Inline drain only** (`snapshot.shared.session=false`): connection slots the monolithic full-table drain (`snapshot.mode` `initial` or `initial_only`, which holds one connection for the whole scan) may not claim, guaranteeing them to streaming/CDC readers so a long drain cannot starve them; range `0` to `max.concurrent.connections − 1`. Only that drain is held off these slots — keyed tables' default `incremental`/`auto_snapshot` snapshots and stream/CDC reads release their slot after each microbatch, so they never starve anyone and **borrow the reserved slots freely**. Default `0`, which for the inline drain is interpreted as **one third of the pool** — `floor(max.concurrent.connections / 3)`, rounded up to at least `1` whenever the pool has 2+ slots (`0` only for a single-slot pool, which cannot spare one) — so a long drain leaves streaming readers about two-thirds of capacity with no tuning. Set a positive value to override. A floor, not a partition — beyond the reserved slots the drain competes for all remaining capacity — so it bounds the starvation without slowing the drain. A value equal to `max.concurrent.connections` is rejected rather than clamped, because it would deadlock the drain. Independent of `snapshot.reader.threads`. |
| `daemon.connection.reservation` | No | `0` | Connection slots the **sharded CDC daemon** may not claim, guaranteeing them to consumer bootstrap/snapshot/incremental reads; range `0` to `max.concurrent.connections − 1`. Unlike a consumer read, a daemon never releases its slot per microbatch — a busy CDC shard re-reads in a tight loop — so without this the CDC daemon can pin **every** slot (its thread count defaults to the pool size and it shards two channels) and a fresh bootstrap read times out and fails the query. Default `0`, interpreted as **one third of the pool** — `floor(max.concurrent.connections / 3)`, rounded up to at least `1` whenever the pool has 2+ slots (`0` only for a single-slot pool). Set a positive value to give the CDC daemon more of the pool (down to reserving a single slot). A floor, not a partition — beyond the reserved slots the CDC daemon uses all remaining capacity. The **snapshot drain daemon** is treated separately: because it must finish to unblock its append-only consumer, it floors one band *below* the CDC daemon (at `daemon.connection.reservation − snapshot.reader.threads`), reserving `snapshot.reader.threads` slots the CDC daemon cannot claim so a saturated CDC daemon cannot starve the drain. Keep `daemon.connection.reservation` **greater than** `snapshot.reader.threads` so a slot also remains below the drain for consumer bootstrap; otherwise the drain floor collapses to `0` and it competes with consumer reads for the CDC-free low band. |
...
- **Changing `scd_type`** between a keyed merge (`SCD_TYPE_1`/`SCD_TYPE_2`) and
  `APPEND_ONLY` — the pipeline swaps a merge flow for an append flow.
...
- **Migrating a table that is still snapshotting.** You do not have to wait for a
  ...
  - **Incremental** (the default: a primary-key-chunked copy interleaved with CDC) is
    ...
    remaining rows.
  - **Blocking `initial`** (`snapshot.mode=initial`, a single bulk read) parks its
    mid-serve position too. Its staged pages are carried **by reference**: the cutover
    records the capturing flow's scope on the offset (`stage_scope`) so the new flow
    reads those already-staged pages **in place** — no copy — finishes the snapshot from
    the parked page, then streams under its own scope. Because the pages are referenced,
    the cutover must happen within `snapshot.staging.retention.days` (the age-based sweep
...
- **Fail-closed on a schema change.** The parked position is tagged with the source
  ...
  position that no longer aligns with the current columns. Re-run the capture step
  against the current schema, or start fresh (`cdc_only`/`initial`).
- **Deletes are carried across the seam.** For a relocation that keeps a keyed
  `scd_type` (a table with a delete channel), the delete reader's position is handed
...
The LSN encoding must match the connector's capture boundary:
`(uniqid << 32) + ((used - 1) << 12)`, the **start** of the current logical log page in
`sysmaster:syslogs` — never its end (`used << 12`), because `used` counts the page being
written, and a commit landing later in that page would otherwise fall at or below the
boundary and be discarded by the stream as already captured.
...
SQL
  ...
  # (uniqid<<32)+((used-1)<<12): the start of the current log page, matching the
  # connector's capture boundary (see the note above the script).
  boundary_lsn=$(( uniqid * 4294967296 + (used > 0 ? used - 1 : 0) * 4096 ))
  rm -f "$lsn_file"
...
# (uniqid<<32)+((used-1)<<12): start of the current log page (the connector's capture boundary).
boundary_lsn=$(( uniqid * 4294967296 + (used > 0 ? used - 1 : 0) * 4096 ))
rm -f "$lsn_file"
printf '{"boundary_lsn": "%s", "boundary_time": "%s"}\n' "$boundary_lsn" "$utc" \
  > "$dir/_manifest.json"
