...
| `daemon.connection.reservation` | No | `0` | Connection slots the background daemons (sharded CDC readers, the snapshot drain pool) may not claim, guaranteeing them to consumer bootstrap/snapshot/incremental reads; range `0` to `max.concurrent.connections − 1`. Unlike a consumer read, a daemon never releases its slot per microbatch — a busy CDC shard re-reads in a tight loop and a drain holds its slot for the whole scan — so without this the daemons can pin **every** slot (the CDC daemon's thread count defaults to the pool size and it shards two channels) and a fresh bootstrap read times out and fails the query. Default `0`, interpreted as **one third of the pool** — `floor(max.concurrent.connections / 3)`, rounded up to at least `1` whenever the pool has 2+ slots (`0` only for a single-slot pool). Set a positive value to give the daemons more of the pool (down to reserving a single slot). A floor, not a partition — beyond the reserved slots the daemons use all remaining capacity. |
