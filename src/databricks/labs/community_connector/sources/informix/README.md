...
| `snapshot.reader.threads` | No | `max(1, max.concurrent.connections // 3)` | **Model C only** (`snapshot.shared.session=true`): number of daemon workers that drain `initial` snapshots concurrently, which is also the ceiling on connection slots snapshots hold at once; minimum `1`. Keyless (append-only, no primary key) tables drain **one-at-a-time** through this pool, so a single worker serializes every keyless table's initial scan and the later tables time out at `snapshot.drain.wait.seconds` before a worker reaches them — the default now scales to a third of the pool (matching the CDC-daemon reservation) to parallelize them out of the box on a 6+-slot pool. Keep it below `max.concurrent.connections` so streaming/CDC readers always retain slots. For a single large keyless table, raising `snapshot.drain.wait.seconds` (a longer scan window) or declaring `primary.keys` (moves it onto the resumable keyed path) is the better lever. |
