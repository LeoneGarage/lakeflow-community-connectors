...
| `cdc.shared.reader.threads` | No | `max.concurrent.connections` | Number of shards / daemon reader threads (K) when `cdc.shared.session=true`; minimum `1`. K=1 collapses a channel onto one session; larger K spreads tables across more parallel readers. Bounds the driver thread count and the concurrent daemon connection slots (which share the pool with consumer bootstrap). |
