...
| `cdc.shared.buffer.max.records` | No | `200000` | Per-table cap on buffered-but-unconsumed transactions in a shard (`cdc.shared.session=true`); minimum `1`. When reached, that shard's daemon defers new reads (throttle to the slowest reader) until the backlog drains, bounding driver memory. |
