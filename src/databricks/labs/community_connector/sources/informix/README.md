...
| `snapshot.prefetch.enabled` | No | `true` | Whether serving a **staged snapshot** prefetches the next page while the current one is written. Each staged page is an immutable Volume blob that is read, decompressed, and decoded on the driver, then written by Spark; when on, a single background daemon thread prepares the next page's rows during the current page's write, so the next microbatch serves it from memory. It holds **no connection slot** and shares no mutable connection with the read path (the page read is pure Volume I/O + CPU on an immutable blob), so the only cost is one extra page resident in driver memory — and a prefetch error falls back to a synchronous read. It never prefetches across the snapshot→stream boundary, and only engages in the staged-serve phase (keyed blocking / keyless `initial` snapshots), **not** the keyed `incremental` path. On by default; set `false` to serve pages synchronously. Accepts `true`/`false` (`1`/`yes`, `0`/`no`). |
