...
| `snapshot.read.timeout.seconds` | No | `300` | SQLI socket read timeout while fetching transactional snapshot pages and while reading the incremental-snapshot key bound (`SELECT FIRST 1 ... ORDER BY <pk> DESC`) that seeds a keyed table; raise it if a large table's key-bound read times out. Minimum `1`. |
