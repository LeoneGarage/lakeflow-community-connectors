...
| `append.only.ingestion` | No | Keyless: `true`; keyed: `false` | CDC-streamable tables without a primary key default to append-only ingestion: the connector reports `ingestion_type=append` with a cursor and streams changes instead of repeatedly reading a bounded snapshot. Set `false` explicitly to opt a keyless table into bounded snapshot-only ingestion. Append semantics are correct only for genuinely insert-only data: updates append another row and deletes leave old rows present. Also set `scd_type: APPEND_ONLY`; the connector cannot read that pipeline setting. Keyed tables may explicitly set `true`. Uncapturable types remain snapshot-only, and append-only tables have no delete channel. |
