...
Lakeflow creates upsert and delete streams independently and does not transfer the snapshot offset between them. Before the first capture, use `prepare_initial_capture()` and configure all three returned options. Both streams require their native table identity in `cdc.initial.prepared.tables` and use the same retained, non-future position; the connector opens/registers/activates a native CDC session without reading LODATA records before returning snapshot rows. Delete initialization fails closed when preparation is absent. Offsets include a schema-layout fingerprint. Before decoding rows after the session's initial METADATA frame, the bridge re-reads the catalog and compares captured membership, type, width, precision, scale, and encoding; another METADATA frame fails immediately. Metadata is also checked after every page/poll, and any changed fingerprint requires a full refresh.

`InformixLakeflowConnect.prepare_initial_capture(table_names=None)` implements the required ordering without source coordination tables: it resolves the selected CDC-capable tables, calls `cdc_set_fullrowlogging(table, 1)` for all of them, reads one current LSN only after every call succeeds, verifies that position against retained/current bounds, and returns `cdc.initial.lsn`, `cdc.initial.fullrow.logging.enabled`, and a JSON `cdc.initial.prepared.tables` list for the first full refresh. Supplying `table_names` limits preparation to exposed `owner.table` names; an explicit empty list fails. Use the connector as a context manager or call `close()` after preparation.
