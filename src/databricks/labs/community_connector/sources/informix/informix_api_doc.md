...
The source timestamp for transaction-boundary records is the Informix BEGIN/COMMIT time (epoch seconds); data envelope processing time is connector time. This commit time is surfaced per row as `_informix_commit_time` (`TimestampType`, UTC) — the COMMIT time for CDC rows, the snapshot boundary time (`DBINFO('utc_current')` at pin, or a file-source manifest `boundary_time`, else NULL) for `r` rows — but at second granularity, so **ordering must still use LSNs, not timestamps**, and `_informix_commit_time` is informational, never `sequence_by`. It is `≤` every later CDC commit time, so the column is monotonic across the snapshot→CDC handoff.
...
encoded as `(uniqid << 32) + (logpage << 12)` (`current_lsn`). `used` is a page count, so that value names the end of the page currently being written and can run up to one page ahead of the true write position. Every position a snapshot is sequenced against — the shared initial boundary, a consistent snapshot's LSN, each incremental chunk's stamp — therefore uses the conservative `(uniqid << 32) + ((logpage - 1) << 12)`, the start of the current page (`boundary_lsn`): a commit landing later in that page after the sample then still sits above the boundary instead of being discarded as already captured. Stop bounds (the AvailableNow high-water mark) and log-file comparisons keep the page-end `current_lsn`, where overshooting is harmless. The boundary is recorded before snapshot data is read, then streaming begins from it so changes concurrent with the snapshot are not missed.
...
### File-sourced initial snapshot (`snapshot.source`)

With `snapshot.source=<Volume path>` and `snapshot.mode=initial`, a table's initial snapshot is served from Informix UNLOAD (`.unl`) files on a Volume instead of a live repeatable-read scan. The connector publishes the same durable initialization/boundary records and emits the shaped rows (`_informix_op="r"`, change/commit LSN = boundary) with a stream-phase offset — so the CDC stream resumes at the boundary and `_recover` discards anything at or below it, identical to a completed live snapshot. A per-record column-count check is the only alignment guard. UNLOAD parsing is `|`-delimited with backslash escaping and empty-field-as-NULL; an *unescaped* physical newline terminates one record (UNLOAD writes an embedded newline as the escape followed by a newline; both the record reader and the byte-split planner keep it inside its record). The extract may equally be Informix High-Performance Loader (`onpload`/`ipload`) **DELIMITED** output, which is the same newline-terminated escaped-delimited shape: the field delimiter and escape character are read from `snapshot.unl.delimiter`/`snapshot.unl.escape` (each a single character, table option then connection, defaulting to UNLOAD's `|`/`\`; an empty escape disables escape processing), so an HPL job's non-`|` delimiter parses without change. The parser already tolerates HPL's missing trailing delimiter (a line with or without one yields the same fields) and `||` is NULL for both. The delimiter/escape ride each `unl_split` partition descriptor so the executor path parses identically to the driver. HPL's fixed-position (COBOL) and Informix-internal binary formats are not supported (not self-describing — they need the ipload map's field widths/types). A physical newline terminating one record lets the reader split large files into ~`snapshot.split.bytes` (default 50 MiB) byte ranges — snapped up to the next record boundary — read in parallel across executors via Files-API ranged GETs (`Range: bytes=…`, 206), falling back to a local seek only when no staging token can be minted. Byte splitting needs no whole-file scan to plan: the planner seeks to each target and reads only a small window to find the next record boundary. The serve is windowed and resumable: `latest_offset`/`get_partitions` advance a split index by `stream.partitioned.snapshot.pages.per.batch` splits per microbatch (reusing the staged-page window and its snapshot-phase offset, with `page_index` as the split index and an empty `last_pk`), transitioning to the stream phase only when the last split is served — so a restart mid-snapshot resumes at the last committed split. The split plan is deterministic (immutable extract + byte target), so the two calls agree on the same global split ordering without shared state. Gzip-compressed extracts (`.unl.gz`, detected by the `1f 8b` magic bytes rather than the name) are supported: because a gzip stream is not seekable, a compressed file is a single non-splittable split that is streamed and decompressed whole (`zlib.decompressobj`, constant memory, concatenated members handled) on one executor, so `snapshot.split.bytes` does not apply to it and parallelism/mid-snapshot resume come from using multiple `.unl.gz` files. Plain and gzip files may be mixed in one directory.

The CDC boundary LSN is resolved per table, first hit wins: (1) the `snapshot.boundary.lsn` option (the LSN captured at extract time); (2) else a `_manifest.json` in the source directory, `{"boundary_lsn": "<X>"}` (other keys ignored); (3) else the connector self-pins its own registration LSN via `prepare_initial_capture`/`_initial_lsn`. An operator-supplied boundary X is authoritative: `initial_lsn == snapshot_lsn == X` is published (not the connector's freshly-captured registration LSN), full-row logging is still enabled idempotently, the boundary is validated at X, and X is range-checked against `minimum_lsn`/`current_lsn` (below the retained minimum fails closed — the logical log recycled past it, a gap). Publishing goes through the same elected/immutable initialization + snapshot-boundary records, so a speculative `latest_offset` re-call is idempotent, and both the CDC stream and the delete-channel bootstrap start at X. The operator is responsible for the extract reflecting the table at the boundary; the connector does not verify alignment. An optional `_manifest.json` `boundary_time` (UTC epoch seconds) becomes the served `r` rows' `_informix_commit_time` (rides each `unl_split` descriptor to the executor path); absent it, those rows carry NULL — a file-source snapshot has no live boundary to query.
...
Staged snapshot pages are served through a **partitioned stream reader** by default (`stream.partitioned=true`). Because each staged page is an immutable, self-describing, independently addressable Volume object, the reader fans a snapshot microbatch out across Spark executors: `latest_offset` advances the page cursor by up to `stream.partitioned.snapshot.pages.per.batch` pages (default `8`), `get_partitions` emits one partition per page in that range, and `read_partition` reads and decodes each page on its own executor — reading only the page's identity (native identity, scope, schema id, snapshot LSN, page index) plus the staging location, so it never opens an Informix catalog connection. This replaces the serial one-page-per-microbatch driver serve for the snapshot phase; the driver still reads one boundary page's header per microbatch to record the resume cursor (`last_pk`). Every other phase is a **single driver-computed partition** whose rows the driver produced through the ordinary `read_table`/`read_table_deletes` path and embedded in the partition descriptor — so bootstrap staging, the live CDC/incremental stream, snapshot→CDC transitions, and the delete channel behave exactly as the simple reader, and **CDC is never parallelized by this reader** (the sharded daemon remains the CDC lever). The reader applies only to tables with a bounded staged-snapshot→stream lifecycle (keyed CDC tables and append-only tables); snapshot-only tables use the simple reader. Both readers persist the same bare offset, so `stream.partitioned` can be toggled on a running pipeline and the existing checkpoint resumes in place with no full refresh — serverless SDP was verified to resume across the simple↔partitioned reader-class swap. `stream.partitioned` **requires `cdc.shared.session=true`** (the default), because an embedded microbatch that Spark re-plans after a driver restart is recomputed from its committed start offset **bounded at its committed end offset** — the recompute must reach exactly that `end` and fails closed on a shortfall, the same contract as the simple reader's `readBetweenOffsets` — and only the shared CDC session's accumulating buffer lets a bounded replay of a range larger than one `cdc.max.records` poll converge (a direct per-table poll is capped per read and could never satisfy the bound). Since `stream.partitioned` defaults on, a config that sets `cdc.shared.session=false` **must** also set `stream.partitioned=false`; otherwise construction fails fast with an explanatory error (there is no silent fallback). Set `stream.partitioned=false` to force the simple single-partition driver reader.
...
Lakeflow creates upsert and delete streams independently and does not transfer offsets between them, so the two channels coordinate through shared state in a Lakebase Postgres endpoint the connector provisions on first use. For each table, only the upsert reader enables full-row logging and publishes initialization, completed-snapshot, triggered-update, and schema-transition boundaries; the delete reader consumes the same records. Each is elected by `INSERT ... ON CONFLICT DO NOTHING RETURNING`, so exactly one writer commits a record and every loser reads back the winner's value. A record cannot be partially visible, so there is no incomplete state to quarantine. The upsert reader also maintains a per-pipeline **delete floor**: the snapshot boundary the current destination rows were captured at, keyed by table and pipeline id (not update scope, so scope cleanup never removes it) and advanced only by a later resnapshot through an `ON CONFLICT ... DO UPDATE ... WHERE existing.start_lsn < excluded.start_lsn` publish. A delete reader with no checkpoint bootstraps at the lower of that floor and the upsert reader's current-scope channel-start, because the channel-start carries the upsert's *resume* position, which drifts past the snapshot across restarts and would otherwise skip the deletes committed in between. A floor below the retained log or from an earlier schema generation is reported and the channel-start used instead.
...
- `get_table_schema()` returns source columns plus a stable internal cursor such as `_informix_change_lsn` (string or decimal(20,0)); optionally include `_informix_commit_lsn`, `_informix_tx_id`, `_informix_op`, and `_informix_commit_time` (TimestampType, the source commit/boundary time) if downstream observability needs them. These correspond to conventional CDC source metadata.
