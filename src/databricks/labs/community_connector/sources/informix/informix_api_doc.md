...
Lakeflow creates upsert and delete streams independently and does not transfer offsets between them. `cdc.shared.state.location` is therefore mandatory and must identify a writable `/Volumes/<catalog>/<schema>/<volume>/...` directory shared by every serverless worker. For each table, only the upsert reader enables full-row logging and atomically publishes initialization, completed-snapshot, triggered-update, and schema-transition boundaries. The delete reader waits for and consumes those same records. The generated source creates one random registration scope before Spark serializes the reader classes; live serverless validation confirmed that all upsert and delete readers receive the same value. Both channels persist it in offset version 7 and use the checkpointed value after restarts. Snapshot and trigger boundaries are keyed by this scope, isolating simultaneous pipeline registrations even though Lakeflow does not expose pipeline/update identity to Python data-source workers. Each invocation acknowledges only the committed start offset; returned end offsets remain unacknowledged until Spark commits and supplies them to a later invocation. Unchanged acknowledgements avoid Volume locking and writes except for a bounded heartbeat. Boundaries acknowledged by both channels are removed. Inactive-scope cleanup is two-phase: one retention interval marks a candidate and a second full interval must pass before removal, preventing a single clock-skewed observation from deleting state. Restarting a removed checkpoint fails closed and requires a full refresh. A completed consistent snapshot advances both channels directly to its fresh snapshot LSN, preventing historical replay and making full refresh recover from an older `TRUNCATE`. Triggered readers checkpoint a shared update-generation ID and use one per-table high-water LSN. Atomic lock directories fail closed after a worker crash and require operator removal only after all pipelines using the connection are stopped; elapsed-time lease reclamation is deliberately avoided because it cannot safely fence a paused owner on Volume storage. Before first use, every worker runs a bounded concurrent client-side probe of exclusive directory creation and rename visibility. This checks that worker's mounted Volume client but does not replace deployment validation with concurrent serverless workers. Lock timeouts expose owner age, PID, exact path, and recovery instructions. Offsets include both a schema-layout fingerprint and a unique schema-node ID. Shared-state version 6 stores scope acknowledgements and removes boundaries from earlier formats while preserving schema history. Shared state stores ID-linked schema generations, Informix catalog table IDs, and their transition LSNs, allowing identical layouts from different table incarnations to occur in separate generations without ambiguity. Its one-MiB serialized bound replaces a fixed schema-node count; reaching that bound requires a coordinated new state location and full refreshes. Lagging readers follow one successor at a time; incompatible full refreshes create independent roots without deleting descriptors still needed by another pipeline. On restart, an appended nullable CDC-supported column is accepted only when every existing descriptor and the primary key remain identical. Both readers drain the old prefix layout through the shared transition, then checkpoint and capture with the expanded descriptor. Other schema changes fail closed and require a full refresh. Before decoding rows after the session's initial METADATA frame, the bridge re-reads the catalog and accepts either exact equality or the unchanged stored prefix during a declared additive transition; another METADATA frame fails immediately.

An active reader may perform a daily sweep across every state file in its hashed connection directory, including tables no longer selected by that pipeline. Atomic creation of a connection-level `.informix-sweep.lock` elects one worker across all pipelines, while a shared `.informix-sweep.json` completion marker makes every other worker honor the same 24-hour interval. The sweep prunes schema branches not reachable from the active schema or retained acknowledgements, tombstones empty state files after two-phase retention, and removes empty directories. If no pipeline ever uses a connection again, connector code cannot execute that sweep; external maintenance remains necessary.
...
Informix requires full-row logging to be disabled for `ALTER TABLE`. Stop every pipeline using the connection first. Application writes must then be quiesced before an operator disables logging and remain quiesced while nullable columns are appended, logging is immediately re-enabled, and the pipeline completes its restart-only schema transition. A transaction spanning the transition fails closed. Values represented through the old capture descriptor appear as null for added columns; update/backfill them after the transition, then resume normal writes. Drops, renames, reorders, existing-column changes, non-nullable/unsupported additions, and primary-key changes remain full-refresh operations.
...
Each `SQ_TUPLE=14` response contains `smallint tuple_warning`, `int32 tuple_payload_length`, exactly that many payload bytes, plus one padding byte when length is odd. Multiple tuple messages may precede EOT. The row decoder uses the describe start positions for fixed layouts. For variable layouts it walks columns sequentially from the tuple start: types `40,41,43,45,46` consume `5 + big_endian_int32(bytes at current+1)`; VARCHAR types `13,16` consume `1 + unsigned length byte` when variable-VARCHAR capability is active; complex types consume `4 + big_endian_int32 length`; other types consume descriptor length. Every computed end must be checked against the complete tuple payload. Describe positions model the fixed layout and do not bound runtime variable envelopes; live LVARCHAR values can legitimately extend beyond the following column's describe position. Scalar values are then decoded by the ordinary JDBC type codecs, not the CDC codec table.
...
`SQ_INSERTDONE=94` is a 10-byte Informix signed-magnitude INT8/SERIAL8 value followed by an 8-byte BIGSERIAL only when protocol feature 54 (bigint support) is enabled. Unknown or out-of-order completion messages fail closed.
...
- `padVarchar=true` selects fixed/padded ordinary SQL decoding only for VARCHAR/NVARCHAR types 13 and 16; it does not alter LVARCHAR, BOOLEAN/extended variable envelopes, CHAR semantics, or native CDC prefixes. The Python tuple walker retains cursor-based layout whenever an LVARCHAR/extended variable field is present and consumes padded VARCHAR widths from their descriptors. Source-local regressions cover padded VARCHAR followed by LVARCHAR and enveloped BOOLEAN, including multibyte client-encoding width checks.
...
Accordingly, the port deliberately rejects complex, INTERVAL, NCHAR, and LOB layouts that are not implemented end to end. INT8/SERIAL8 use the corrected ten-byte signed-magnitude representation. LVARCHAR uses the five-byte ordinary SQLI variable envelope for snapshots and its native length/flag representation for CDC. These types have source-local golden coverage, including mixed LVARCHAR/BOOLEAN tuple layout. `ansi_live_test.py` provides an opt-in `scalar.types.live.validation` regression requiring a populated table containing INT8, SERIAL8, DATETIME, LVARCHAR, and BOOLEAN columns. It passed against Informix 15.0.FC0DE over verified TLS in an ANSI database, using a reversible non-scalar marker update to force full-row boundary and null CDC records. Optional paired INSERT-smoke and cleanup statements exercise the command-path `SQ_INSERTDONE` SERIAL8/BIGSERIAL completion metadata without changing the deterministic seed rows. Supported packed DECIMAL/DATETIME and CDC lifecycle paths have source-local and live validation; unsupported layouts must continue to fail closed.
...
Current verification includes the CFR/bytecode audit, the source-local protocol/connector regression suite, and a disposable Informix 15 fixture. The pure-Python client completed normal-password association, enhanced-protocol setup, DBOPEN, feature-62 PREPARE, fixed and variable cursor fetches (including extended `SQ_RET_TYPE` negotiation), table/column/primary-key discovery, padded and unpadded scalar snapshots, and committed full-row CDC updates. Live scalar coverage includes INT8, SERIAL8, DATETIME, LVARCHAR, BOOLEAN, and their nullable wire forms. The live DATETIME result confirmed JDBC's decimal-container rule: the descriptor's high byte is total precision, its low byte contains the qualifier, and wire width is `IfxDecimal.decLength(encodedLength)`. Production requires verified TLS for password authentication. No credentials were persisted.
