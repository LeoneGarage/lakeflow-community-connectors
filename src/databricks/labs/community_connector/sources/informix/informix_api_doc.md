...
An Informix LSN is a signed 64-bit decimal string in Debezium offsets and source metadata. Its official form is `LSN(loguniq,logpos_hex)`. `-1`/`NULL` means unavailable. Ordering compares the 64-bit sequence. The restart position is a four-part transaction-log position:
...
This yields at-least-once delivery around a Lakeflow checkpoint boundary. Only return an `end_offset` after a complete committed transaction has been decoded and all returned rows are materialized. If a call fails, return nothing/raise and leave the prior offset unchanged. A projected Lakeflow batch cap may be exceeded to avoid splitting a committed transaction. The lower native read must nevertheless stop at its configured record bound or the first native `TIMEOUT` control frame, even when the final transaction is open; discard that incomplete transaction for the poll, keep the prior checkpoint, and replay from the retained begin LSN next time. Treating a timeout frame as ordinary non-empty data or waiting indefinitely for commit prevents Trigger Available Now pipelines from terminating. When Spark calls `prepareForTriggerAvailableNow()`, the connector-specific stream adapter notifies the Informix connector to freeze the current LSN at stream startup and ignore later commits for that run. Spark does not make that callback in continuous mode, so continuous execution follows new commits without a mode-specific option.
...
For Lakeflow, snapshot pagination should be deterministic by the complete primary key (`ORDER BY pk`, seek after `last_pk`). Tables without a primary/unique key cannot be safely seek-paginated or deduplicated under CDC; either reject them by default or explicitly run snapshot-only. Record the pre-snapshot high-water LSN in the first offset and do not replace it with a post-snapshot value.
...
For Lakeflow SCD Type 2, `sequence_by` controls both event ordering/deduplication and the values and data types of `__START_AT` and `__END_AT`. Sequencing by the default `_informix_change_lsn` therefore produces string validity columns containing decimal LSNs. A table may instead select a non-null source timestamp such as `updated_at` to obtain timestamp validity columns, but that timestamp then replaces the LSN as the Auto CDC ordering key. It must change on every source mutation and be precise enough to order repeated changes to one primary key. Informix CDC extraction and recovery still checkpoint native LSNs independently of the downstream Auto CDC sequence. Existing targets must be recreated or fully refreshed when the sequence type changes. `__END_AT IS NULL` denotes the current active version.
...
The implementation and its 23 synthetic tests are derived from the exact IBM JDBC 15.0.1.3, change-stream client 1.1.5, and Debezium Informix 3.6.0.Final bytecode documented below. This establishes a serverless-capable design, not production readiness. No live Informix record/session has yet validated the reconstructed wire behavior; the evidence gaps, excluded data types, and excluded authentication modes below remain binding limitations.
...
The current Python path has two independent mismatches that explain a timeout before the first query response: `encode_prepare` always emits a two-byte SQL length even when feature 62 is active, shifting the SQL and all following codes; and `execute` flushes OPEN and waits for a response before sending the first FETCH even for fixed-width descriptors. Correct behavior is capability-aware SQL encoding followed by descriptor-driven OPEN/FETCH grouping. Prepared binds belong between CURNAME and OPEN, exactly as shown.
...
Debezium 3.6.0.Final's `InformixStreamingChangeEventSource` explicitly removes native types BYTE `11`, TEXT `12`, UDT variable `40`, UDT fixed `41`, UNKNOWN `49`, and anything for which `IfxTypes.isComplexType` is true. IBM defines complex as SET `19`, MULTISET `20`, LIST `21`, and ROW `22`. This predicate does **not** exclude INTERVAL `14`, INT8 `17`, SERIAL8 `18`, LVARCHAR `43`, BOOLEAN `45`, BIGINT `52`, or BIGSERIAL `53`; however change-stream client 1.1.5's `IfxCDCOperationRecord` has no INTERVAL branch, so an included INTERVAL still fails at decode. BLOB/CLOB extended types use native extended mappings and are not safe merely because their public IDs are `101/102`; their capture descriptor resolves through unsupported extended/locator behavior.
...
`IfxInt8.fromIfx` invokes `IfxToJavaLongInt` over all ten bytes. Decode `sign=int16(bytes[0:2])`; sign `0` is null, sign `1` or `-1` selects the sign of `(uint32(bytes[6:10]) << 32) | uint32(bytes[2:6])`; reject every other sign. The current Python `be_int64(bytes[0:8])` plus trailer allow-list model is incompatible and must not be enabled. The `Long.MIN_VALUE` returned internally for null is a Java sentinel, not an on-wire eight-byte sentinel. The magnitude of `-2^63` also needs a checked special case because Java's `l = -l` overflows in the encoder. SERIAL8 has the same value domain but generated-key semantics do not alter a CDC row value.
...
- VARCHAR/LVARCHAR: VARCHAR `(length=1, byte=0)` is null. LVARCHAR null/empty are `00 01 01`/`00 01 00`, and nonempty data begins at byte 3. Current Python misses the VARCHAR sentinel and is one byte early for LVARCHAR.
...
- Query execution: negotiated feature 62 changes command/prepare SQL to a four-byte length, and fixed-width cursors combine OPEN plus initial FETCH before the first response. Current Python uses a two-byte SQL length and waits between OPEN/FETCH; both are live-proven timeout causes. `SQ_COMMAND` exists but is not JDBC's SELECT path.
...
Accordingly, the pure-Python code is not yet a type-compatible port even after association fixes: its INT8 decoder is structurally wrong, complex/INTERVAL/LOB exclusions remain necessary, and live differential fixtures are still required for packed DECIMAL/DATETIME and CDC lifecycle behavior.
