...
This yields at-least-once delivery around a Lakeflow checkpoint boundary. Only return an `end_offset` after a complete committed transaction has been decoded and all returned rows are materialized. If a call fails, return nothing/raise and leave the prior offset unchanged. A batch cap may be exceeded to avoid splitting a transaction.
